  ...
  return (
    <section className="action-dock" aria-label="行动">
      ...
      <div className="action-buttons">
        <button disabled={locked} onClick={() => dispatch({ type: 'FOUND_CAMP' })}><span>⌂</span> 建营 <small>8 金</small></button>
        <button onClick={() => dispatch({ type: 'STATION_FOLLOWER' })} disabled={locked || followers === 0}><span>⚑</span> 驻守</button>
        <button disabled={locked} onClick={() => dispatch({ type: 'REST' })}><span>☾</span> 休息</button>
      </div>
